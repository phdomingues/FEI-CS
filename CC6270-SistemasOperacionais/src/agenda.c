...
int i=0;
void* show_external(void* ag_){
	system("gnome-terminal -e 'python ./show_ag.py'");
	pthread_exit(NULL);
}
...
void mostrar_agenda(struct agenda ag){
	...
	int rc = pthread_create(&thread, NULL, show_external, (void*)&ag);
	i++;
	if (rc){
...
int main(int argc, char* argv[]){
	...
	printf("Carregar agenda (1) / Criar uma nova agenda (2): ");
	scanf("%d",&option);
	struct agenda ag;
	if (option == 1){
