...
input_H2O = Input("H20", 0.2, 0.2, 1, 1, stop_signal)
...
# === Secadores - Perda de 3%
big_dryer = Dryer(math.inf, "Secador Maior",0 , 0.03, stop_signal)
...
pipe_reactor_decanter = Pipe("Pipe(NaOh/2xEtOH/Oleo)", decanter)
pipe_decanter_etoh = Pipe("Pipe(EtOH)", tank_etoh)
...
pipe_h2o_washing_tank_1 = Pipe("Pipe(H20)", washing_tank_1)
pipe_washing_tank_3_big_dryer = Pipe("Pipe (Solução Lavada)", big_dryer)
pipe_big_dryer_tank_biodiesel = Pipe("Pipe (Solução Secada)", tank_biodisel)
...
input_H2O.connect_pipe(pipe_h2o_washing_tank_1)
washing_tank_3.connect_pipe(pipe_washing_tank_3_big_dryer)
big_dryer.connect_pipe(pipe_big_dryer_tank_biodiesel)
# Logging tool
log_thread = threading.Thread(target=logging, name="logging_tool", args=(stop_signal, tank_oil, tank_naoh_etoh, tank_glycerin, tank_biodisel, tank_etoh, reactor, decanter, washing_tank_1, washing_tank_2, washing_tank_3, big_dryer, tank_biodisel))
...
threads.append(input_H2O.start())
threads.append(washing_tank_3.start())
threads.append(big_dryer.start())
#threads.append(washing_tank_3.start())
